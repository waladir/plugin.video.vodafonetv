...
class Channels:
    ...
    def get_channels(self):
        ...
        channels = {}
        session = Session()
        # post = {"language":"ces","ks":session.ks,"filter":{"objectType":"KalturaSearchAssetFilter","kSql":""},"pager":{"objectType":"KalturaFilterPager","pageSize":300,"pageIndex":1},"clientTag":clientTag,"apiVersion":apiVersion}
        post = {"language":"ces","ks":session.ks,"filter":{"objectType":"KalturaChannelFilter","kSql":"","idEqual":346867},"pager":{"objectType":"KalturaFilterPager","pageSize":300,"pageIndex":1},"clientTag":clientTag,"apiVersion":apiVersion}
        result = list_api(post = post)
        for channel in result:
            if 'Channel number' in channel['metas']:
                ...
                if len(channel['images']) > 1:
                    ...
                    image = None
                channels.update({int(channel['id']) : {'channel_number' : int(channel['metas']['Channel number']['value']), 'vdf_number' : int(channel['metas']['Channel number']['value']), 'name' : channel['name'], 'id' : channel['id'], 'logo' : image, 'visible' : True}})
        return channels
    ...
    def merge_channels(self):
        vdf_channels = self.get_channels()
        max_number = 0
        if len(self.channels) > 0:
            max_number = self.channels[max(self.channels, key = lambda channel: self.channels[channel]['channel_number'])]['channel_number']
        for channel in sorted(vdf_channels, key = lambda channel: vdf_channels[channel]['channel_number']):
            if channel in self.channels:
                if self.channels[channel]['name'] != vdf_channels[channel]['name']:
                    self.channels[channel].update({'name' : vdf_channels[channel]['name']})
                if self.channels[channel]['vdf_number'] != vdf_channels[channel]['vdf_number']:
                    self.channels[channel].update({'vdf_number' : vdf_channels[channel]['vdf_number']})
                if self.channels[channel]['logo'] != vdf_channels[channel]['logo']:
                    self.channels[channel].update({'logo' : vdf_channels[channel]['logo']})
            else:
                max_number = max_number + 1
                vdf_channels[channel]['channel_number'] = max_number
                self.channels.update({channel : vdf_channels[channel]})
        for channel in list(self.channels):
            if channel not in vdf_channels:
